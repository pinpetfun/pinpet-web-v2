    ...
    try {
        ...
        if (!mint || typeof mint !== 'string') {
            return {
                ...
                errorMessage: '代币地址必须是有效的字符串 / Token address must be a valid string',
                data: null
            };
        }
        ...
        if (sellTokenAmount === undefined || sellTokenAmount === null) {
            return {
                success: false,
                errorCode: 'PARAM_ERROR',
                errorMessage: '卖出Token数量不能为空 / Sell token amount cannot be empty',
                data: null
            };
        }
        ...
        try {
            ...
            return {
                success: false,
                errorCode: 'PARAM_ERROR',
                errorMessage: `无效的Token数量: ${error.message} / Invalid token amount: ${error.message}`,
                data: null
            };
        }

        // 2. 获取当前价格 / Get current price
        ...
        try {
            ...
            if (!currentPriceData.success || !currentPriceData.data || currentPriceData.data.length === 0) {
                return {
                    success: false,
                    errorCode: 'API_ERROR',
                    errorMessage: '无法获取代币价格信息 / Cannot get token price information',
                    data: null
                };
            }
        } catch (error) {
        ...
        if (targetReachedAtSegmentIndex !== -1) {
            ...
            if (theoreticalTradeResult) {
                const [, _theoreticalSolObtained] = theoreticalTradeResult;
                // 更新理论SOL数量
            }
        }
