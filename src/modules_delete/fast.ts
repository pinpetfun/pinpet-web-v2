...
/**
 * Fast API 模块
 * 从中心化服务器中 用API接口访问数据，快速拿到交易所需的参数
 * 可以流流畅的进行交易
 */
class FastModule {
  private sdk: any;
  ...
  constructor(sdk: any) {
    ...
    if (!this.baseUrl) {
      throw new Error('FastModule 需要配置 spinFastApiUrl');
    }
  ...
  /**
   * 统一数据获取入口 - 智能代理模式
   * @param {string} type - 数据类型 ('mints', 'orders', 'price' 等)
   * @param {Object} options - 请求参数
   * @returns {Promise<Object|DataManager>} 根据类型返回数据或管理器实例
   */
  async get(type, options = {}) {
    ...
    if (config.useManager) {
      ...
      if (!config.ManagerClass) {
        throw new Error(`数据管理器 ${type} 尚未实现`);
      }
   ...
   * @param {string} options.sort_by - 排序方式，默认'slot_asc'
  ...
  async orders(mint, options = {}) {
    const params = {
      mint: mint,
      type: (options as any).type || 'down_orders',
      page: (options as any).page || 1,
      limit: (options as any).limit || 500,
      ...options
    };
  ...
  async user_orders(user, mint, options = {}) {
    const params = {
      ...
      mint: mint,
      page: (options as any).page || 1,
      limit: (options as any).limit || 200,
      order_by: (options as any).order_by || 'start_time_desc',
      ...options
    };
