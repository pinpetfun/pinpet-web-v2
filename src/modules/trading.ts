import { ComputeBudgetProgram, PublicKey, Transaction, SystemProgram, SYSVAR_RENT_PUBKEY } from '@solana/web3.js';
...
class TradingModule {
  ...
  /**
   * 买入代币
   * @param {Object} params - 买入参数
   * @param {string|PublicKey} params.mintAccount - 代币铸造账户地址
   * @param {anchor.BN} params.buyTokenAmount - 要购买的代币数量
   * @param {anchor.BN} params.maxSolAmount - 最大 SOL 花费
   * @param {PublicKey} params.payer - 支付者公钥
   * @param {Object} options - 可选参数
   * @param {number} options.computeUnits - 计算单元限制，默认 1400000
  ...
  async buy({ mintAccount, buyTokenAmount, maxSolAmount, payer }, options = {}) {
    const { computeUnits = 1400000 } = options as any;
    ...
    if (!anchor.BN.isBN(buyTokenAmount) || !anchor.BN.isBN(maxSolAmount)) {
      throw new Error('buyTokenAmount 和 maxSolAmount 必须是 anchor.BN 类型');
    }
    ...
    return {
      transaction,
      signers: [], // 买入交易不需要额外的签名者，只需要 payer 签名
      accounts: {
        mint: mint,
        curveAccount: accounts.curveAccount,
        poolTokenAccount: accounts.poolTokenAccount,
  ...
  /**
   * 卖出代币
   * @param {Object} params - 卖出参数
   * @param {string|PublicKey} params.mintAccount - 代币铸造账户地址
   * @param {anchor.BN} params.sellTokenAmount - 要卖出的代币数量
   * @param {anchor.BN} params.minSolOutput - 最小 SOL 输出
   * @param {PublicKey} params.payer - 支付者公钥
   * @param {Object} options - 可选参数
   * @param {number} options.computeUnits - 计算单元限制，默认 1400000
   * @returns {Promise<Object>} 包含交易对象、签名者和账户信息的对象
  ...
  async sell({ mintAccount, sellTokenAmount, minSolOutput, payer }, options = {}) {
    ...
    if (!anchor.BN.isBN(sellTokenAmount) || !anchor.BN.isBN(minSolOutput)) {
      throw new Error('sellTokenAmount 和 minSolOutput 必须是 anchor.BN 类型');
    }
    ...
    return {
      transaction,
      signers: [], // 卖出交易不需要额外的签名者，只需要 payer 签名
      accounts: {
        mint: mint,
        curveAccount: accounts.curveAccount,
        poolTokenAccount: accounts.poolTokenAccount,
  ...
  /**
   * 保证金做多
   * @param {Object} params - 做多参数
   * @param {string|PublicKey} params.mintAccount - 代币铸造账户地址
   * @param {anchor.BN} params.buyTokenAmount - 要购买的代币数量
   * @param {anchor.BN} params.maxSolAmount - 最大 SOL 花费
   * @param {anchor.BN} params.marginSol - 保证金数量
   * @param {anchor.BN} params.closePrice - 平仓价格
   * @param {PublicKey} params.payer - 支付者公钥
   * @param {Object} options - 可选参数
   * @param {number} options.computeUnits - 计算单元限制，默认 1400000
   * @returns {Promise<Object>} 包含交易对象、签名者和账户信息的对象
  ...
  async long({ mintAccount, buyTokenAmount, maxSolAmount, marginSol, closePrice, prevOrder, nextOrder, payer }, options = {}) {
    ...
    if (!anchor.BN.isBN(buyTokenAmount) || !anchor.BN.isBN(maxSolAmount) ||
      ...
      throw new Error('所有参数必须是 anchor.BN 类型');
    }
    const currentPrice = await this.sdk.data.price(mintAccount);
    ...
    // // 10. 获取 prevOrder 和 nextOrder（简化计算）
    // // 这里使用简化的逻辑，实际项目中可以根据需要调用模拟器
    // const prevOrder = null; // 可以根据需要计算
    // const nextOrder = null; // 可以根据需要计算
    ...
    return {
      transaction,
      signers: [], // 做多交易不需要额外的签名者，只需要 payer 签名
      accounts: {
        mint: mint,
        curveAccount: accounts.curveAccount,
        poolTokenAccount: accounts.poolTokenAccount,
