...
/**
 * 转换订单数据格式
 * @param {Object} ordersData - 原始订单数据
 * @returns {Array} 转换后的订单数组
 */
 ...
 * @property {number} order_type - 订单类型 (e.g., 1 for down_orders, 2 for up_orders).
 * @property {bigint} lock_lp_start_price - 锁定的流动性起始价格。
 * @property {bigint} lock_lp_end_price - 锁定的流动性结束价格。
...
function checkPriceRangeOverlap(order_type, order_list, lp_start_price, lp_end_price) {
  ...
  function checkLiquidityReservationOverlap(checkOrder) {
    ...
    let _expandedStart, expandedEnd,hasOverlap;
    if (isDown) {
      // down_orders: start不变，end向下扩大
      _expandedStart = orderMax;
...
export { transformOrdersData, checkPriceRangeOverlap };